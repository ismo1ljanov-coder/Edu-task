...
export async function createAnnouncement(userIds: string[], title: string, message: string) {
  return prisma.notification.createMany({
    data: userIds.map((userId) => ({ userId, type: 'ANNOUNCEMENT' as const, title, messa
cat backend/src/services/notification.service.ts
git add -A
git commit -m "Rewrite notification.service.ts cleanly"
git push
cat > backend/src/services/notification.service.ts << 'EOF'
import { prisma } from '../config/prisma';
